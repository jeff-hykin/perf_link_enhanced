import { html, css, uid, startTesting, latestLocalStorage, updateTestCaseName, removeTestCase, addTestCase, copyTestCase, updateTestCaseCode, highlightCode } from "../utils.js"

import { CopyIcon, CloseIcon, RunIcon, ForkIcon, SaveIcon, AddIcon } from "./icons.js"
...
export const TestControls = ({ id, test, state, dispatch }) => {
    ...
    return html`
        <div className=${style.testHeader}>
            ...
            <input disabled=${started} className=${style.nameInput} onInput=${(e) => dispatch(updateTestCaseName(id, e.target.value))} value=${`${test.name}`} />
            <p>${test.ops !== -2 && (test.ops === -1 ? "Failed" : test.ops === 0 ? `Testing ${progressPercent}%` : `${Number(test.ops).toLocaleString("en")} ops/s`)}</p>
            <button disabled=${started} className=${style.button} onClick=${(e) => dispatch(copyTestCase(id))}>
                <${CopyIcon} />
            </button>
            <button disabled=${started} className=${style.button} onClick=${(e) => dispatch(removeTestCase(id))}>
                <${CloseIcon} />
            </button>
        </div>
    `
    ...
    return html`
        <article className="tests">
            ...
            <div className=${style.testToolbar}>
                ...
                <button disabled=${started} data-animate=${dialog} className=${style.start} onClick=${() => dispatch(startTesting)}>
                    <span>Run Tests</span>
                    <${RunIcon} />
                </button>
                <button
            ...
            <div className=${style.testToolbar}>
                <h3>Test Cases</h3>
                <div>
                    <button className=${style.add} disabled=${started} onClick=${() => dispatch(addTestCase)}>
                        <span>Add Case</span>
                        <${AddIcon} />
                    </button>
                </div>
                ...
                ${tests.map(
                    (test, id) => html`
                        <li key=${id} className=${style.test}>
                            <${TestControls} id=${id} test=${test} state=${state} dispatch=${dispatch} />
                            <${Editor} key=${id} value=${test.code} disabled=${started} onValueChange=${(code) => dispatch(updateTestCaseCode(id, code))} highlight=${highlightCode} padding=${20} style=${style.editor} />
                        </li>
                    `
                )}
            </ul>
